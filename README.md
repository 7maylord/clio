...
- **👁️ Visually Impaired Profile:** Focuses on literal descriptions — spatial orientation, colors, shapes, brushstroke texture (such as Van Gogh's thick _impasto_), and physical dimensions.
...
The artwork database (`catalog.json`) contains **9 pieces** spanning **6 paintings** and **3 sculptures** — from Van Gogh's _Starry Night_ to Michelangelo's _David_ and the _Venus de Milo_. Each entry includes detailed history, visual description, technique, dimensions, and gallery location. A **FastMCP server** (`mcp_server.py`) wraps the lookup tool for local development via stdio transport.
